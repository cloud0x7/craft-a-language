...
func NewSemanticAnalyer() *SemanticAnalyer {
	...
	// ret.passes = append(ret.passes, NewTrans())
	return ret
}
...
func (e *Enter) VisitForStatement(stmt *ForStatement, additional any) any {
	oldScope := e.scope
	e.scope = NewScope(e.scope)
	stmt.Scope = e.scope

	e.AstVisitor.VisitForStatement(stmt, additional)
	e.scope = oldScope
	return nil
}
...
func (e *RefResolver) VisitForStatement(stmt *ForStatement, additional any) any {
	oldScope := e.scope
	e.scope = stmt.Scope
	if e.scope == nil {
		fmt.Println("Scope 不可为nil")
	}
	e.declaredVarsMap[e.scope] = map[string]*VarSymbol{}
	e.AstVisitor.VisitForStatement(stmt, additional)
	e.scope = oldScope
...
func (lv *LeftValueAttributor) VisitUnary(exp *Unary, additional any) any {
	if exp.Op == scanner.Inc || exp.Op == scanner.Dec {
		lastParentOperator := lv.ParentOperator
		lv.ParentOperator = exp.Op

		lv.Visit(exp.Exp, additional)
		if !exp.Exp.IsLeftValue() {
			lv.addError(fmt.Sprintf("unary operator %s can only be appled to a left value", exp.Op.ToString()), exp)
		}

		lv.ParentOperator = lastParentOperator
	} else {
		lv.AstVisitor.VisitUnary(exp, additional)
	}
	return nil
}
...
func (t *TypeChecker) VisitVariableDecl(variableDecl *VariableDecl, additional any) any {
	...
	if variableDecl.Init != nil {
		...
		t2 := variableDecl.Init.GetTheType()
		if !t2.LE(t1) {
...
func (t *TypeChecker) VisitBinary(exp *Binary, additional any) any {
	...
	t1 := exp.Exp1.GetTheType()
	t2 := exp.Exp2.GetTheType()
	...
	} else if scanner.IsRelationOp(exp.Op) {
		if t1.LE(Number) && t2.LE(Number) {
			exp.TheType = Boolean
		} else {
			t.addError(fmt.Sprintf("operator '%s' can not be applied to %s and %s.\n", exp.Op.ToString(), t1.(*SimpleType).Name, t2.(*SimpleType).Name), exp)
		}
...
func (e *Trans) VisitProg(prog *Prog, additional any) any {
	prog.Stmts = append(prog.Stmts, NewReturnStatement(prog.endPos, prog.endPos, nil, false))
	return nil
}
