...
func (p *Parser) parseStatement() IStatement {
	...
	} else if t.Code == scanner.If {
		return p.parseIfStatement()
	} else if t.Code == scanner.For {
...
func (p *Parser) parseIfStatement() (rt *IfStatement) {
	beginPos := p.scanner.GetNextPos()
	...
	if p.scanner.Peek().Code == scanner.OpenParen { // '(')
		p.scanner.Next()
		condition = p.parseExpression()
		if p.scanner.Peek().Code == scanner.CloseParen { // ')'
			p.scanner.Next()
		} else {
			p.addError("Expect ')' after if condition.", p.scanner.GetLastPos())
			p.skip(nil)
			isErrorNode = true
		}
	} else {
		p.addError("Expecting '(' after 'if'.", p.scanner.GetLastPos())
		p.skip(nil)
		condition = NewErrorExp(beginPos, p.scanner.GetLastPos())
	}
	// 解析then语句
	stmt := p.parseStatement()
	var elseStmt IStatement

	if p.scanner.Peek().Code == scanner.Else {
		p.scanner.Next()
		elseStmt = p.parseStatement()
	}

	return NewIfStatement(beginPos, p.scanner.GetLastPos(), condition, stmt, elseStmt, isErrorNode)
}

func (p *Parser) parseForStatement() (rt *ForStatement) {
	beginPos := p.scanner.GetNextPos()
	// 跳过for
	p.scanner.Next()
	...
	var terminate IExpression
	var increment IExpression

	if p.scanner.Peek().Code == scanner.OpenParen {
		p.scanner.Next()

		if p.scanner.Peek().Code != scanner.SemiColon {
			if p.scanner.Peek().Code == scanner.Let {
				p.scanner.Next()
		...
		if p.scanner.Peek().Code == scanner.SemiColon {
			...
			p.addError("Expecting ';' after init part of for statement.", p.scanner.GetLastPos())
			p.skip(nil)
		...
		// terminate
		if p.scanner.Peek().Code != scanner.SemiColon {
			terminate = p.parseExpression()
		}
		if p.scanner.Peek().Code == scanner.SemiColon {
			p.scanner.Next()
		} else {
			p.addError("Expecting ';' after terminate part of for statement.", p.scanner.GetLastPos())
			p.skip(nil)
			if p.scanner.Peek().Code == scanner.SemiColon {
				p.scanner.Next()
			}
		...
		if p.scanner.Peek().Code == scanner.CloseParen {
			...
			p.addError("Expecting ')' after increment part of for statement.", p.scanner.GetLastPos())
			p.skip(nil)
			if p.scanner.Peek().Code == scanner.CloseParen {
				p.scanner.Next()
			}
		...
		p.addError("Expecting '(' after 'for'.", p.scanner.GetLastPos())
		p.skip(nil)
		isErrorNode = true
	}

	stmt := p.parseStatement()
	return NewForStatement(beginPos, p.scanner.GetLastPos(), init, terminate, increment, stmt, isErrorNode)
}
...
func (p *Parser) parseUnary() (rt IExpression) {
	...
	if t.Kind == scanner.Operator {
		...
		if t1.Kind == scanner.Operator && (t1.Code == scanner.Inc || t1.Code == scanner.Dec) {
			p.scanner.Next()
			return NewUnary(beginPos, p.scanner.GetLastPos(), t1.Code.(scanner.OP), exp, false, false)
		} else {
			return exp
		}
	}
